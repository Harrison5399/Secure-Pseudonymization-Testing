...
import type { EnvelopeBundle } from "./types";
...
/**
 * Minimal IndexedDB helper for encrypted payload persistence.
 */
export async function saveEncryptedItem<T>(storeName: string, key: string, value: T): Promise<void> {
  const db = await openDB("harrison-client-encryption", 1, {
    upgrade(database) {
