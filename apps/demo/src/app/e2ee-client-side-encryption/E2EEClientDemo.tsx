...
function parsePacket(content: string): E2EEPacket | null {
  try {
    ...
    if (
      ...
      return value as unknown as E2EEPacket;
    }
...
export default function E2EEClientDemo({ sessionUser }: Props) {
  ...
  async function sendE2EEMessageByEmail() {
    ...
    try {
      ...
      const local = ring?.keys.find((key) => key.keyId === ring.activeKeyId) ?? ring?.keys[0];
      const deviceId = getOrCreateDeviceId(sessionUser.id);
    ...
    try {
      ...
      for (const copy of uniquePrioritizedCopies) {
        ...
        try {
          ...
          failureReasons.push(`copy ${copy.keyId} failed: ${error instanceof Error ? error.message : "decrypt error"}`);
        }
      }
